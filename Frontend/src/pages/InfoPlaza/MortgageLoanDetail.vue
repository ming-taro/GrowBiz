        ...
        >
          ...
          <ul class="list-unstyled fs-sm ms-1 mb-4">
            ...
            <li class="d-flex align-items-center mb-2">
              ...
              ><span>{{ data.lendRateMin }} ~ {{ data.lendRateMax }} %</span>
            </li>
          </ul>
          <hr class="mb-4" />

          <h3 class="h5 pt-2">상세 내용</h3>
          <ul class="list-unstyled">
            <li class="d-flex">
              <span class="text-primary fs-lg me-2">&#8226;</span>
              <span class="label">평균 금리</span>
              <span class="colon">: </span>
              <span class="ms-1">{{ data.lendRateAvg }}%</span>
            </li>
            <li class="d-flex">
              <span class="text-primary fs-lg me-2">&#8226;</span>
              <span class="label">대출 한도</span>
              <span class="colon">: </span>
              <span class="ms-1">{{ data.loanLmt }}</span>
            </li>
            <li class="d-flex">
              <span class="text-primary fs-lg me-2">&#8226;</span>
              <span class="label">대출 부대비용</span>
              <span class="colon">: </span>
              <span class="ms-1">{{ data.loanInciExpn }}%</span>
            </li>
            <li class="d-flex">
              <span class="text-primary fs-lg me-2">&#8226;</span>
              <span class="label">중도상환 수수료</span>
              <span class="colon">: </span>
              <span class="ms-1">{{ data.erlyRpayFee }}%</span>
            </li>
          </ul>
        </div>
...
const bringDataList = async () => {
  try {
    // Best 인기 업종 - 전체
    const response = await axios.get(BASEURI + '/getDetailItemMortgage', {
      params: {
        id: id.value,
...
const bringRecommandList = async () => {
  try {
    // Best 인기 업종 - 전체
    console.log(data.value);
    const response = await axios.get(BASEURI + '/getFilteredMortgageList', {
      params: {
        bankName: '전체',
        repayMethod: data.value.rpayTypeNm,
        type: data.value.lendRateTypeNm,
      }, // 선택된 필터링 값을 쿼리 파라미터로 전송
    });
    if (response.status === 200) {
